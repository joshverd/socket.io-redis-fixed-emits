...
function adapter(uri, opts) {
  ...
  Redis.prototype.onmessage = function(pattern, channel, msg){
    ...
    // Custom fix for edge case global emits
    if(channel === 'socket.io#/#/#') {
      channel = 'socket.io#/#';
      if(args[2]) {
        if(args[2].rooms) {
          if(args[2].rooms[0]) {
            if(args[2].rooms[0] === '/' && args[2].rooms.length === 1) {
              console.log('Fixed emit!');
              args[2].rooms = [ ];
            }
          }
        }
    ...
    if (room !== '' && room !== '/' && !this.rooms.hasOwnProperty(room)) {
  ...
  Redis.prototype.onrequest = function(channel, msg){
    channel = channel.toString();
    var request = request = JSON.parse(msg);
  ...
  Redis.prototype.broadcast = function(packet, opts, remote){
    /*if(packet.data[0] === 'banner' || packet.data[0] === 'withdraws') {
      console.log('Found accepted socket emit.', packet, remote);
    }*/
